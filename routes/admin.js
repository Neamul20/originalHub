...
router.post('/seller-applications/:userId/approve', ...isAdmin, async (req, res) => {
  try {
    ...
    await db.query(
      "UPDATE seller_profiles SET is_approved=TRUE, approved_at=NOW() WHERE user_id=$1",
      [req.params.userId]
    );
  ...
  try {
    ...
    if (action === 'delete_product' && r.product_id) {
      ...
      if (product.rows.length) {
        await db.query('UPDATE users SET is_banned=TRUE WHERE id=$1', [product.rows[0].seller_id]);
      }
      ...
      if (product.rows.length) {
        ...
        if (parseInt(cnt.rows[0].c) >= 3) {
          ...
          await db.query(
            ...
            [until, product.rows[0].seller_id]
          );
        }
      }
    }
...
router.get('/users', ...isAdmin, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT id, email, full_name, role, is_banned, is_suspended, suspended_until, created_at, last_login FROM users ORDER BY created_at DESC'
    );
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
